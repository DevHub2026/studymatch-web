...
function TutorCard({ tutor, index, onRequest, requested }) {
  ...
  const dept  = tutor.department || tutor.course || ''
  const rating = parseFloat(tutor.rating || tutor.average_rating || 0)
  const reviews = tutor.reviews_count || tutor.total_reviews || 0
  const subjects = tutor.subjects || tutor.teach_subjects || []
  const availability = tutor.availability || ''
  const bio   = tutor.bio || tutor.about || ''
  ...
  return (
    ...
    >
      ...
      <div style={{ flexShrink: 0, textAlign: 'right', display: 'flex', flexDirection: 'column', gap: 8, alignItems: 'flex-end' }}>
        ...
        <button
          onClick={() => onRequest(tutor.id || tutor.user_id)}
          disabled={requested}
          style={{
...
export default function FindTutorsPage() {
  ...
  const filtered = tutors.filter(t => {
    ...
    const dept = t.department || t.course || ''
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {filtered.map((tutor, i) => (
                <TutorCard
                  ...
                  requested={!!requested[tutor.id || tutor.user_id]}
                />
              ))}
            </div>
          )}
