...
export default function AdminDashboardPage() {
  ...
  useEffect(() => {
    const load = async () => {
      setLoading(true)
      try {
        const [statsRes, tutorsRes, reportsRes] = await Promise.allSettled([
          api.get('/admin/stats'),
        ...
        if (statsRes.status === 'fulfilled') {
          ...
          setStats({
            ...
            tutors:  d.total_tutors  || d.tutors  || 0,
            pending: d.pending_verifications || d.pending || 0,
            reports: d.total_reports || d.reports || 0,
          })
        }

        if (tutorsRes.status === 'fulfilled') {
          const d = tutorsRes.value?.data?.data || tutorsRes.value?.data || []
  ...
  return (
    <div style={{ fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
        ...
        <div style={{ background: '#fff', borderRadius: 20, padding: 24, border: '1px solid #E5E7EB' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
            <h2 style={{ fontSize: 18, fontWeight: 700, color: '#111827', margin: 0 }}>Recent Reports</h2>
            <Link to="/admin/reports" style={{ fontSize: 13, color: '#7C3AED', fontWeight: 600, textDecoration: 'none' }}>View all</Link>
          </div>
          ...
          {recentReports.length === 0 ? (
            ...
            const category = report.category || report.type || 'Report'
            const status   = report.status || 'Pending review'
            return (
              <div key={report.id || i} style={{ padding: '14px 0', borderBottom: i < recentReports.length - 1 ? '1px solid #F3F4F6' : 'none' }}>
                <div style={{ fontWeight: 600, color: '#111827', marginBottom: 6 }}>{category}</div>
