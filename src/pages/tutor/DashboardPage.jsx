...
export default function TutorDashboardPage() {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (partnersRes.status === 'fulfilled') {
          const data = partnersRes.value?.data || partnersRes.value || []
          setStudents(Array.isArray(data) ? data : [])
        }
        if (requestsRes.status === 'fulfilled') {
          const data     = requestsRes.value?.data || requestsRes.value || {}
          const received = data.received || data || []
          setRequests(Array.isArray(received) ? received.filter(r => r.status === 'pending') : [])
          const accepted = Array.isArray(received) ? received.filter(r => r.status === 'accepted').length : 0
          setWeekStats(p => ({ ...p, matches: accepted }))
        }
      } catch {}
  ...
  const handleAccept  = async (id) => {
    try { await acceptMatchRequest(id); setRequests(p => p.filter(r => r.id !== id)) } catch {}
  }
        ...
        <div className="td-main">
          ...
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 4 }}>
              <div>
                <div style={{ fontWeight: 700, fontSize: 17, color: '#1E1B4B' }}>Recommended Matches</div>
                <div style={{ fontSize: 13, color: '#9CA3AF', marginTop: 2 }}>Students who are a great match for your expertise.</div>
              </div>
              <Link to="/tutor/find-students" style={{ fontSize: 13, color: '#7C3AED', fontWeight: 600, textDecoration: 'none' }}>View all</Link>
            </div>
            ...
            <div style={{ background: 'white', border: '1px solid #F0F0F4', borderRadius: 16, overflow: 'hidden', marginTop: 14 }}>
              {loading ? (
                ...
                <div style={{ padding: '40px 24px', textAlign: 'center' }}>
                  ...
                  <div style={{ fontWeight: 700, fontSize: 15, color: '#374151', marginBottom: 6 }}>No student matches yet</div>
                  <div style={{ fontSize: 13, color: '#9CA3AF', marginBottom: 14 }}>Students looking for tutors will appear here.</div>
                  <Link to="/tutor/find-students" style={{ color: '#7C3AED', fontWeight: 600, fontSize: 13, textDecoration: 'none' }}>Browse all students →</Link>
                </div>
              ) : (
                visibleStudents.map((s, i) => (
                  <StudentCard key={s.id || i} student={s} index={i} saved={savedIds.includes(s.id)} onSave={toggleSave} />
                ))
              )}
            </div>
            ...
            {students.length > 3 && (
              <div style={{ textAlign: 'center', marginTop: 14 }}>
                <button onClick={() => setShowAll(v => !v)} style={{
                  ...
                  {showAll ? 'Show less' : 'View more students'}
                  <ChevronDown size={15} style={{ transform: showAll ? 'rotate(180deg)' : 'none', transition: '.2s' }} />
                </button>
              </div>
            )}
          </div>
            ...
            {requests.length === 0 ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                {requests.slice(0, 5).map((r, i) => {
                  const name  = r.sender?.name || r.user?.name || r.name || 'Student'
